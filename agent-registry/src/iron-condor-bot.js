...
// stop / near expiry. TESTNET by default (from the user's stored creds.testnet).
//
// Honest + safe: only TRADE-only keys, small configurable size, no return promises.
// Real execution must be validated on Deribit TESTNET before mainnet.
...
function createState(owner, cfg) {
  return {
    owner, status: 'running', config: defaults(cfg),
...
async function botTick(state, getCreds, save) {
  const st = state; const cfg = st.config;
  ...
  try {
    ...
    // Sync our option positions from the exchange.
    ...
    if (!st.open) {
      ...
      // Place the 4 legs (market for fills). sell shorts, buy wings.
      const placed = [];
      ...
      if (okLegs.length === 4) {
        ...
        save && save({ type: 'open', expiry: sig.expiry, credit_usd: Number(st.creditUsd.toFixed(2)), legs: st.legs, reason: 'Iron Condor opened' });
      } else {
        // Partial fill → unwind what filled (safety) and report.
    ...
    // floating P&L is in the base coin (BTC); convert with the position's index via mark — approximate with sum of floating_pl * spot.
    const floatingBtc = ours.reduce((s, p) => s + (Number(p.floating_pl) || 0), 0);
    // value in USD ≈ floating (BTC) * (mark of any leg's underlying). We approximate with creditUsd basis.
    st.currentPnlUsd = Number((floatingBtc * (st.creditUsd && ours.length ? 1 : 1)).toFixed(2)); // floating already ~USD-ish on Deribit PM; report raw
    // If positions vanished (expired/closed externally) → reset.
    if (ours.length === 0) {
    ...
    if (exit) {
      // Close: reverse each leg reduce-only market.
      ...
      save && save({ type: exit, reason: `Closed (${exit})`, pnl_usd: st.currentPnlUsd });
      resetOpen(st);
    } else {
...
// Deribit expiry code like "28JUN26" → ms until 08:00 UTC on that date.
...
export function startBot(owner, cfg, getCreds, save, intervalMs = 20_000) {
  stopBot(owner);
  const state = createState(owner, cfg);
  const loop = setInterval(() => { if (state.status === 'running') botTick(state, getCreds, save).catch((e) => { state.lastError = String(e?.message || e); }); }, Math.max(10_000, intervalMs));
  if (loop.unref) loop.unref();
  botTick(state, getCreds, save).catch(() => {});
...
export function getBotStatus(owner) {
  ...
  const s = e.state;
  return { running: s.status === 'running', open: s.open, asset: s.config.asset, testnet: true, expiry: s.expiry, credit_usd: Number(s.creditUsd.toFixed(2)), pnl_usd: s.currentPnlUsd, legs: s.legs, last_action: s.lastAction, last_error: s.lastError, ticks: s.tickCount, config: s.config };
}
